...
describe("postponeExam", () => {
  it("moves publishAt forward and extends expiresAt by the release window", async () => {
    const { blob, cleanup } = await freshModules();
    await blob.writeExamData("e1", exam("e1", "2026-08-18T13:00:00Z"));
    await blob.writeManifest([entry("e1", "2026-08-18T13:00:00Z")]);

    const ok = await cleanup.postponeExam("e1", "2026-08-18T15:00:00Z");

    expect(ok).toBe(true);
    const manifest = await blob.readManifest();
    expect(manifest[0]?.publishAt).toBe("2026-08-18T15:00:00.000Z");
    expect(manifest[0]?.expiresAt).toBe("2026-08-18T20:00:00.000Z");
    const data = await blob.readExamData("e1");
    expect(data?.publishAt).toBe("2026-08-18T15:00:00.000Z");
    expect(data?.expiresAt).toBe("2026-08-18T20:00:00.000Z");
  });

  it("returns false and writes nothing for an unknown exam", async () => {
    const { blob, cleanup } = await freshModules();
    await blob.writeManifest([entry("e1", "2026-08-18T13:00:00Z")]);
    const spy = vi.spyOn(blob, "writeManifest");

    const ok = await cleanup.postponeExam("nope", "2026-08-18T15:00:00Z");
